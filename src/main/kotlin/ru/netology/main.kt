...
fun main() {
    outTransferFee(amount = 200 * 100)
    outTransferFee(amount = 20000 * 100)
    outTransferFee(amount = 10000 * 100, monthSum = 35000 * 100)
    println();
    outTransferFee(amount = 200 * 100, type = MMCARD)
    outTransferFee(amount = 20000 * 100, monthSum = 20000 * 100, type = MMCARD)
    outTransferFee(amount = 10000 * 100, monthSum = 595000 * 100, type = MMCARD)
    outTransferFee(amount = 10000 * 100, daySum = 145000 * 100, type = MMCARD)
    println();
    outTransferFee(amount = 200 * 100, type = VISAMIR)
    outTransferFee(amount = 20000 * 100, monthSum = 20000 * 100, type = VISAMIR)
    outTransferFee(amount = 10000 * 100, monthSum = 595000 * 100, type = VISAMIR)
    outTransferFee(amount = 10000 * 100, daySum = 145000 * 100, type = VISAMIR)
...
fun outTransferFee(type: String = VKPAY, monthSum: Int = 0, daySum: Int = 0, amount: Int) {
    when {
        type == VKPAY && amount > maxTransferVK -> {
            println("Сумма перевода Vk превышает установленный лимит")
        }
        type == VKPAY && (monthSum + amount) > maxMonthTransferVK -> {
            println("Сумма перевода Vk превышает лимит в месяц")
        }
        type !== VKPAY && (daySum + amount) > maxDayTransferCard -> {
            println("Сумма перевода по карте превышает дневной лимит")
        }
        type !== VKPAY && (monthSum + amount) > maxMonthTransferCard -> {
            println("Сумма перевода по карте превышает лимит в месяц")
        }
        else -> {
            ...
            println("Вы совершатете перевод в системе $type")
            println(
